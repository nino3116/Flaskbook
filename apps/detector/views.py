...
def exec_detect(target_image_path):
    ...
    # 이미지 복사처 경로를 취득한다.
    detected_image_file_path = str(Path(current_app.config["UPLOAD_FOLDER"],
...
def delete_image(image_id):
    try:
        # user_image_tags 테이블로붑터 레코드를 삭제한다
...
def search():
    # 이미지 일람을 가져온다
    user_imgages = db.session.query(User, UserImage).join(UserImage, User.id == UserImage.user_id)
    ...
    for user_image in user_imgages:
        ...
        if not search_text:
            ...
            # 검색 단어로 추출한 태그를 가져온다
            user_image_tags = (
                db.session.query(UserImageTag).filter(UserImageTag.user_image_id == user_image.UserImage.id)
